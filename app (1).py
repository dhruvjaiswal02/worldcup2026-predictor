...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📊 Championship Odds", "🗂️ Stage Breakdown", "🌍 Groups", "📈 Team Ratings", "⚡ Upset Radar", "🏆 Live Bracket"])
...
with tab6:
    st.markdown("### 🏆 Live Tournament Bracket")
    st.markdown("Simulate a full tournament run and watch every match unfold — group stage through to the Final. Click **Simulate Again** for a different outcome every time.")

    if st.button("🔄 Simulate Full Tournament", key="bracket_sim"):
        random.seed(None)
        ...
        def first(g):  return group_results[f"Group {g}"][0]
        def second(g): return group_results[f"Group {g}"][1]
        def third_t(g): return (group_results[f"Group {g}"][2], group_pts[f"Group {g}"][group_results[f"Group {g}"][2]])

        all_thirds = [(g[-1], third_t(g[-1])[0], third_t(g[-1])[1]) for g in GROUPS.keys()]
        best8 = sorted(all_thirds, key=lambda x: x[2], reverse=True)[:8]
        thirds_by_letter = {x[0]: x[1] for x in best8}
        best8_teams = [x[1] for x in best8]
        ...
        key = frozenset([x[0] for x in best8])
        mapping = THIRD_MATRIX.get(key, {})
        _ti = iter(best8_teams)
        def get_third(wl):
            tg = mapping.get(wl)
            if tg and tg in thirds_by_letter: return thirds_by_letter[tg]
            return next(_ti, best8_teams[0])
        ...
        for gi, (gname, teams) in enumerate(GROUPS.items()):
            with gcols[gi % 4]:
                ranked = group_results[gname]
                pts    = group_pts[gname]
                st.markdown(f"<div style='background:#0d1b35;border:1px solid #1a2a4a;border-radius:8px;padding:8px;margin-bottom:8px;'><div style='color:#f0d060;font-size:0.7rem;font-weight:700;letter-spacing:0.1em;border-bottom:1px solid #1a2a4a;padding-bottom:4px;margin-bottom:6px;'>{gname.upper()}</div>{''.join([f"<div style='display:flex;justify-content:space-between;padding:2px 0;font-size:0.75rem;color:{'#6ab4f0' if i==0 else '#5ae890' if i==1 else '#e0a040' if i in [2] and t in best8_teams else '#4a6a8a'};'><span>{'🥇' if i==0 else '🥈' if i==1 else '🟡' if t in best8_teams else '❌'} {t}</span><span style='font-weight:700;'>{pts[t]}pt</span></div>" for i,t in enumerate(ranked)])}</div>", unsafe_allow_html=True)
        ...
        def show_round(matches, round_name, emoji):
            winners = []
            st.markdown("---")
            st.markdown(f"#### {emoji} {round_name}")
            n = len(matches)
            cols_per_row = min(4, n)
            for row_start in range(0, n, cols_per_row):
                batch = matches[row_start:row_start+cols_per_row]
                rcols = st.columns(len(batch))
                for ci, (t1, t2) in enumerate(batch):
                    w1p, d, w2p = predict_match(t1, t2)
                    winner = simulate_knockout(t1, t2)
                    loser  = t2 if winner == t1 else t1
                    winners.append(winner)
                    with rcols[ci]:
                        winner_pct = w1p*100 if winner==t1 else w2p*100
                        loser_pct  = w2p*100 if winner==t1 else w1p*100
                        st.markdown(f"""
                        <div style='background:#0d1b35;border:1px solid #1a2a4a;border-radius:8px;overflow:hidden;margin-bottom:6px;'>
                          <div style='background:#0d2d18;padding:6px 10px;display:flex;justify-content:space-between;align-items:center;'>
                            <span style='color:#5ae890;font-weight:700;font-size:0.85rem;'>&#9989; {winner}</span>
                            <span style='color:#5ae890;font-size:0.7rem;'>{winner_pct:.0f}%</span>
                          </div>
                          <div style='padding:6px 10px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid #1a2a4a;'>
                            <span style='color:#3a5a6a;font-size:0.8rem;text-decoration:line-through;'>&#10060; {loser}</span>
                            <span style='color:#3a5a6a;font-size:0.7rem;'>{loser_pct:.0f}%</span>
                          </div>
                        </div>
                        """, unsafe_allow_html=True)
            return winners

        r32_winners = show_round(r32_matches, "Round of 32", "⚔️")
        r16_winners = show_round([(r32_winners[i], r32_winners[i+1]) for i in range(0, 16, 2)], "Round of 16", "🔥")
        qf_winners  = show_round([(r16_winners[i], r16_winners[i+1]) for i in range(0, 8, 2)], "Quarter-Finals", "⭐")
        sf_winners  = show_round([(qf_winners[i],  qf_winners[i+1])  for i in range(0, 4, 2)], "Semi-Finals", "🌟")
        champion    = show_round([(sf_winners[0],  sf_winners[1])],   "Final", "🏆")

        # ── Champion ─────────────────────────────────────────────────────
        st.markdown("---")
        st.markdown(f"""
        <div style='text-align:center;padding:2rem;background:linear-gradient(135deg,#0d2d18,#1a3a2a);border:2px solid #f0d060;border-radius:12px;margin-top:1rem;'>
          <div style='font-size:3rem;'>🏆</div>
          <div style='font-family:Bebas Neue,sans-serif;font-size:0.8rem;color:#4a9a6a;letter-spacing:0.2em;text-transform:uppercase;margin-top:4px;'>FIFA World Cup 2026 Champion</div>
          <div style='font-family:Bebas Neue,sans-serif;font-size:3rem;color:#f0d060;letter-spacing:0.1em;line-height:1.1;'>{champion[0]}</div>
          <div style='font-size:0.75rem;color:#4a7a5a;margin-top:6px;'>Adj Elo: {RATINGS.get(champion[0],1500)}</div>
        </div>
        """, unsafe_allow_html=True)

    else:
        st.markdown("""
        <div style='text-align:center;padding:3rem;background:#0d1b35;border:1px dashed #1a2a4a;border-radius:12px;color:#4a7aaa;'>
          <div style='font-size:3rem;margin-bottom:1rem;'>⚽</div>
          <div style='font-size:1.1rem;font-weight:600;color:#8ab8e8;'>Click "Simulate Full Tournament" to run a complete World Cup</div>
          <div style='font-size:0.85rem;margin-top:0.5rem;'>Every group match, every knockout round, all the way to the Final</div>
        </div>
        """, unsafe_allow_html=True)
